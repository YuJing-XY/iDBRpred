...
foreach $file (@files) {
	...
	$fname = $tmp;
	if ($model eq "X") {
		system("./bin/disbinX ./model_definition/ensembleCX $fname.disbin $fname.flatblast $thres | awk \'\$1==\"D\" || \$1==\"O\"\' > $basename.espritz");
		print "Finished executing XRAY calpha disorder no psi-blast and threshold=$thres\n";
	}
	elsif ($model eq "D") {
		system("./bin/disbinD ./model_definition/ensembleD $fname.disbin $fname.flatblast $thres | awk \'\$1==\"D\" || \$1==\"O\"\' > $basename.espritz");
		print "Finished executing DISPROT disorder no psi-blast and threshold=$thres\n";
	}
	elsif ($model eq "N") {
		system("./bin/disbinN ./model_definition/ensembleN $fname.disbin $fname.flatblast $thres | awk \'\$1==\"D\" || \$1==\"O\"\' > $basename.espritz");
		print "Finished executing NMR calpha disorder no psi-blast and threshold=$thres\n";
	}
	elsif ($model eq "pX") {
		system("./align/getAlignments.pl $fname");
		system("./bin/disbin_psi ./model_definition/ensembleCX_psi $fname.disbin $fname.flatblast $thres | awk \'\$1==\"D\" || \$1==\"O\"\' > $basename.espritz");
		print "Finished executing XRAY calpha disorder WITH psi-blast and threshold=$thres\n";
	}
	elsif ($model eq "pN") {
		system("./align/getAlignments.pl $fname");
		system("./bin/disbin_psi ./model_definition/ensembleN_psi $fname.disbin $fname.flatblast $thres | awk \'\$1==\"D\" || \$1==\"O\"\' > $basename.espritz");
		print "Finished executing NMR disorder WITH psi-blast and threshold=$thres\n";
	}
	elsif ($model eq "pD"){
		system("./align/getAlignments.pl $fname");
		system("./bin/disbin_psi ./model_definition/ensembleD_psi $fname.disbin $fname.flatblast $thres | awk \'\$1==\"D\" || \$1==\"O\"\' > $basename.espritz");
